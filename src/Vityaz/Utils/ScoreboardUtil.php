...
class ScoreboardUtil {
    ...
    public function setHubScoreboard(Player $player, string $title = "Vityaz"): void {
        ...
        $this->lineCreate($player, 0, str_repeat("§7  ", 9));
        $this->lineCreate($player, 1, "§cOnline: §f" . $totalCount);
        $this->lineCreate($player, 2, "§cHub: §f" . count($this->core->getServer()->getOnlinePlayers()));
        $this->lineCreate($player, 3, str_repeat(" ", 2));
        $this->lineCreate($player, 4, "§cPing: §f" . $player->getPing() . "ms");
        $this->lineCreate($player, 5, str_repeat("  ", 9));
